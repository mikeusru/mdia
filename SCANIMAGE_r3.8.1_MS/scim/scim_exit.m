...
global state gh gui af spc spcs yphys % Misha - 2-19-15 add gui handle for spc figures
...
for i=1:length(guiHandles)  
    if isfield(gh.(guiHandles{i}), 'figure1') && ishandle(gh.(guiHandles{i}).figure1) %VI091510A %Misha - check if figure1 field exists
        delete(gh.(guiHandles{i}).figure1);
    elseif isfield(gh.(guiHandles{i}), 'figure') % Misha - delete figures if they are nested in 'figure' struct
        subHandles=fieldnames(gh.(guiHandles{i}).figure);
        for j=1:length(subHandles)
            if isfield(gh.(guiHandles{i}), subHandles{j}) && ishandle(gh.(guiHandles{i}).(subHandles{j}))
                delete(gh.(guiHandles{i}).(subHandles{j}));
            end
        end
    end
end

%Clear remaining FLIM, SPC, and AF windows - MISHA - 021915
FLIM_Close;
if isfield(gh, 'spc') && isfield(gh.spc, 'FLIMimage') && ishandle(gh.spc.FLIMimage.figure1)
delete(gh.spc.FLIMimage.figure1);
end
if isfield(gui,'spc')
    guiHandles=fieldnames(gui.spc.figure);
    for i=1:length(guiHandles)
        if ishandle(gui.spc.figure.(guiHandles{i}))
            delete(gui.spc.figure.(guiHandles{i}));
        end
    end
    
    if isfield(gui.spc, 'spc_main') && ishandle(gui.spc.spc_main.spc_main)
        delete(gui.spc.spc_main.spc_main);
    end
end
if isfield(gh, 'yphys') && isfield(gh.yphys, 'stimScope') && ishandle(gh.yphys.stimScope.figure1)
    delete(gh.yphys.stimScope.figure1);
end
if isfield(af,'handles_afGUI') && ishandle(af.handles_afGUI.figure1) %close autofocus window
    delete (af.handles_afGUI.figure1);
end
...
clear global gh state gui af spc spcs yphys;
