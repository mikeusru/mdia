function saveUncagingInfo
%saveUncagingInfosaves a text file of the uncaging information during a
%staggered uncaging session. it's mostly to get the timing of the uncaging
%event.
%

global state ua

filename=state.files.savePath;
a=clock;
if ~state.files.autoSave %make sure autosave is turned on
    return
end

fid=fopen([filename,'\UncagingInfo.txt'],'wt'); %create textfile
% fprintf(fid,'%s%s\n','Capture type: ',origin);
