...
%         redval(i, j) = mean2(redim);
        ...
        greenim = filter2(ones(3,3)/9, greenim);
        redim = medfilt2(redim, [3,3]);
        redim = filter2(ones(3,3)/9, redim);
...
for j = 1 :nRoi
    ...
    plot(A1.roiData{j}.Gmax, '-o', 'color', color_a{j});
    hold on;
    subplot(2,2,2);
    if j == 1
        hold off
    end
    plot(A1.roiData{j}.Rmax, '-o', 'color', color_a{j});
    hold on;
        subplot(2,2,3);
    if j == 1
        hold off
    end
    plot(A1.roiData{j}.Rave, '-o', 'color', color_a{j});
    hold on;
        subplot(2,2,4);
    if j == 1
        hold off
