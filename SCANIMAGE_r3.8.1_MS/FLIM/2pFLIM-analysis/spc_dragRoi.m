...
if (gcf == gui.spc.figure.lifetimeMap)
    hf = gui.spc.figure.lifetimeMap;
    ha = gui.spc.figure.lifetimeMapAxes;
    ho = gui.spc.figure.mapRoi;
else
    hf = gui.spc.figure.project;
    ha = gui.spc.figure.projectAxes;
    ho = gui.spc.figure.roi;
end

point1 = get(ha,'CurrentPoint'); % button down detected
point1 = point1(1,1:2);              % extract x and y

RoiRect = get(ho, 'Position');
rectFigure = get(hf, 'Position');
rectAxes = get(ha, 'Position');
Xlim1 = get(ha, 'Xlim');
Ylim1 = get(ha, 'Ylim');
...
Xlim = get(ha, 'YLim')+0.5;
Ylim = get(ha, 'Xlim')+0.5;
