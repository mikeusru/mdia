classdef RS232DeviceBasic < most.DClass
    ...
    properties (SetAccess=protected)
        ...
        defaultTimeout = .5; % Default response timeout used for every receive command
    end    
...
%                 disp(['Command Sent: ' command]); %MISHA
            else % numeric vector
                precision = class(command);
                fwrite(obj.hSerial,command,precision);
%                 disp('Command Sent: '); %MISHA
%                 disp(command); %MISHA
%                 disp(precision); %MISHA
            end
        end
