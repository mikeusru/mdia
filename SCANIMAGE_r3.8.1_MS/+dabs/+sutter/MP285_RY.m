classdef MP285 < dabs.interfaces.LSCSerial
    ...
    methods
        ...
        function tf = get.isMoving(obj)
            tf = 0; %obj.hRS232.isAwaitingReply();
        end
        ...
        function v = get.positionAbsoluteRaw(obj)
            ...
            fclose(obj.hRS232.hSerial);
            posn = dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 2, 'c');
            %disp(length(posn));
            for i=1:2
                if length(posn) == 1
%                     if posn == -4
%                         fprintf('No signal received %d. Try again \n', i);
%                     else
%                         warning ('Received error signal. Trying again');                       
%                     end
                    pause(0.1);
                    dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 2, [char(3), 'a']);
                    pause(0.1);
                    posn = dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 2, 'c');
                    if length(posn) == 3
                        fprintf('Signal received\n');
                    end
                    %disp(posn);
                else
    ...
    methods (Access=private)
        ...
        function val = zprpGetStatusProperty(obj,statusProp)
            fclose(obj.hRS232.hSerial);
            status = dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 0, 's'); %obj.getStatus();
        ...
        function zprpSetVelocityAndResolutionOnDevice(obj)
            ...
            fclose(obj.hRS232.hSerial);
            dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 0, 'va', commandValue);
        end
    ...
    methods (Access=protected,Hidden)
        ...
        function moveStartHook(obj,absTargetPosn)
            ...
            fclose(obj.hRS232.hSerial);
            dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, obj.moveTimeout, 'ma', absTargetPosn);
            %assert(0, 'moveStartHook does not work for now!!');
...
%             obj.hRS232.sendCommandSimpleReply(int32(absTargetPosn),'timeout',obj.moveTimeout);
            ...
            fclose(obj.hRS232.hSerial);
            errorCode = dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, obj.moveTimeout, 'ma', absTargetPosn);
            if errorCode == 2
                disp('Time out. Stop the movement and reading the position');
                dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 2, [char(3), 'a']);
                pause(0.1);
            end
        end
...
%             obj.hRS232.readStringRaw(); % read CR response
        ...
        function resetHook(obj)
            ...
            dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 0, 'r');
        ...
        function zeroHardHook(obj,coords)
           ...
           fclose(obj.hRS232.hSerial);
           dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 0, 'oa');
           %obj.hRS232.sendCommand('o')
        end
...
%             obj.hRS232.readBinaryRaw(1); % trailing CR
            status = dabs.sutter.MP285Commander(obj.hRS232.hSerial.port, obj.hRS232.hSerial.BaudRate, 0, 's');
