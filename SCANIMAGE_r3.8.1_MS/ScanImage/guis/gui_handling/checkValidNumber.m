...
% function to check if entry in GUI is valid
    ...
    if ~strcmpi(get(handle,'Type'),'uicontrol')
        return;
    end
...
% 			val=str2num(get(handle, 'String'));
	...
	elseif strcmp(style, 'edit')
		if hasUserDataField(handle,'NumericAllowEmpty')
			return;
		elseif hasUserDataField(handle, 'LastValid')
			set(handle, 'String', getUserDataField(handle, 'LastValid'));
		elseif hasUserDataField(handle,'NumericString')
			return;
		else		
			set(handle, 'String', '0');
			out=0;
