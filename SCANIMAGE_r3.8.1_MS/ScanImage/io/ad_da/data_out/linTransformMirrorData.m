...
if dia.acq.doRibbonTransform %misha - don't scale data if doing ribbon transform
   scaledMirrorDataOutput = state.acq.mirrorDataOutputOrg;
else
...
if dia.init.etl.etlOn %% MISHA
    makeEtlDataOutput(size(scaledMirrorDataOutput,1));
    etlScale=dia.etl.acq.mirrorSlowShiftScale;
    scaledMirrorDataOutput(:,2)=scaledMirrorDataOutput(:,2)*etlScale;
    
    try
        if dia.etl.acq.doMirrorTransform
            zOffset=round(etlVoltToMotorZCalc(dia.etl.acq.voltageMin));
            scaledMirrorDataOutput=geometricAberrationCompensation(scaledMirrorDataOutput,dia.etl.acq.Htransform(:,:,zOffset));
%             mirrorTransformX=polyval(dia.etl.acq.mirrorTransformPolyX,dia.etl.acq.voltageMin);
%             mirrorTransformY=polyval(dia.etl.acq.mirrorTransformPolyY,dia.etl.acq.voltageMin);
%             scaledMirrorDataOutput(:,1)=scaledMirrorDataOutput(:,1)*mirrorTransformX;
%             scaledMirrorDataOutput(:,2)=scaledMirrorDataOutput(:,2)*mirrorTransformY;
        end
    end
end
...
if isfield(dia.init,'doBeamPowerTransform') && dia.init.doBeamPowerTransform
    buildPockelsModulationMatrix; %Misha
end
