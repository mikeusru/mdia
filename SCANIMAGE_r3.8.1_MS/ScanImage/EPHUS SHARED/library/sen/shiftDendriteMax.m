...
for i=1:1
    ...
    if (sizeofaxon>7)
        ...
        voxel(1,:)=medfilt(voxel(1,:),14);
        voxel(2,:)=medfilt(voxel(2,:),14);
        voxel(3,:)=medfilt(voxel(3,:),14);
        voxel(1,:)=smooth(voxel(1,:),7)';
        voxel(2,:)=smooth(voxel(2,:),7)';
        voxel(3,:)=smooth(voxel(3,:),7)';
    end
...
%         disp(['2. RMS error for step ' num2str(i) ': ' num2str(RMS)]);
        ...
        if (sizeofaxon>7)
            ...
            % smoothing again to futher suppress jumps
            voxel(1,:)=medfilt(voxel(1,:),20);
            voxel(2,:)=medfilt(voxel(2,:),20);
            voxel(3,:)=medfilt(voxel(3,:),20);
